...
def create_expr(tensors):
    strs = [" ".join([VOCAB_TOKENS[y] for y in x.tolist()]) for x in tensors]
    parsed = []
    for s in strs:
        s = re.sub(r"\[\w+\]", "", s)
        s = re.sub(r"_pi_", f"{round(Math.pi, ROUND)}", s)
        s = re.sub(r"_e_", f"{round(Math.e, ROUND)}", s)
        s = re.sub(r"_", "", s)
